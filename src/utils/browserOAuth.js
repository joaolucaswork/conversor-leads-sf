...
async function getOAuthConfig() {
  ...
  const response = await fetch('http://localhost:8000/api/v1/oauth/config', {
    method: 'GET',
    headers: {
  ...
  return new Promise((resolve, reject) => {
    ...
    const messageHandler = (event) => {
      ...
      if (event.data.type === 'OAUTH_SUCCESS') {
        ...
        popup.close();
        oauthState.popup = null;
        reject(new Error(event.data.error.error || 'OAuth authentication failed'));
      }
    };
...
export async function exchangeCodeForToken(code) {
  ...
  try {
    ...
    // Get code verifier
    const codeVerifier = oauthState.codeVerifier || browserStorage.get('oauth_code_verifier');
    ...
    console.log(`Browser OAuth: Token exchange completed in ${processingTime}ms`);
    ...
    console.error(`Browser OAuth: Token exchange failed after ${processingTime}ms:`, error);
    
    // Enhanced error message for expired codes
    if (error.message.includes('expired authorization code') || 
        error.message.includes('invalid_grant')) {
      throw new Error(`Authorization code expired (processed in ${processingTime}ms). Please try logging in again.`);
    }
...
export async function processOAuthCallback() {
  try {
    ...
    console.log('Browser OAuth: Processing callback with code:', code.substring(0, 10) + '...');
