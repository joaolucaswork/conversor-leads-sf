...
export const loginToSalesforce = async () => {
  try {
    if (
      !window.electronAPI ||
      typeof window.electronAPI.getSalesforceAuthUrl !== "function"
    ) {
      throw new Error("Salesforce API is not available on window.electronAPI");
    }
...
export const exchangeCodeForToken = async (code) => {
  try {
    if (
      !window.electronAPI ||
      typeof window.electronAPI.salesforceExchangeCode !== "function"
    ) {
      throw new Error(
        "Salesforce API for code exchange is not available on window.electronAPI"
      );
    }
...
export const refreshToken = async () => {
  try {
    if (
      !window.electronAPI ||
      typeof window.electronAPI.salesforceRefreshToken !== "function"
    ) {
      throw new Error(
        "Salesforce API for token refresh is not available on window.electronAPI"
      );
    }
...
export const logoutFromSalesforce = async () => {
  try {
    if (
      !window.electronAPI ||
      typeof window.electronAPI.salesforceLogout !== "function"
    ) {
      throw new Error(
        "Salesforce API for logout is not available on window.electronAPI"
      );
    }
...
export const getStoredSalesforceAuth = async () => {
  try {
    if (
      !window.electronAPI ||
      typeof window.electronAPI.getStoreValue !== "function"
    ) {
...
export const isTokenExpired = (authData, bufferSeconds = 300) => {
  if (!authData || !authData.accessToken || !authData.issuedAt) {
    return true; // If any crucial part is missing, assume expired/invalid.
  }

  // If expiresIn is missing, use default expiration time (7200 seconds = 2 hours)
  const DEFAULT_EXPIRES_IN = 7200;
  const expiresIn = authData.expiresIn || DEFAULT_EXPIRES_IN;

  // Log if we're using default expiration
  if (!authData.expiresIn) {
    console.warn(
      "Token expiration check: expiresIn missing, using default:",
      DEFAULT_EXPIRES_IN
    );
  }

  const currentTime = Math.floor(Date.now() / 1000); // Current time in Unix seconds
  const expiryTime = authData.issuedAt + expiresIn;
...
export const onSalesforceOAuthCallback = (handler) => {
  if (
    window.electronAPI &&
    typeof window.electronAPI.onSalesforceOAuthCallback === "function"
  ) {
    window.electronAPI.onSalesforceOAuthCallback(handler);
  } else {
    console.warn(
      "electronAPI.onSalesforceOAuthCallback is not defined. Ensure preload script exposes it."
    );
  }
};
...
export const onSalesforceOAuthError = (handler) => {
  if (
    window.electronAPI &&
    typeof window.electronAPI.onSalesforceOAuthError === "function"
  ) {
    window.electronAPI.onSalesforceOAuthError(handler);
  } else {
    console.warn(
      "electronAPI.onSalesforceOAuthError is not defined. Ensure preload script exposes it."
    );
  }
};
