...
# -*- coding: utf-8 -*-
"""
Automatic migration script for Heroku release phase
Safely initializes database tables without manual intervention
"""
...
def safe_database_init():
    ...
    Designed for automatic execution in Heroku release phase
    """
    try:
        ...
        if not database_url:
            print("⚠️  [AUTO-MIGRATE] No DATABASE_URL found, skipping migration")
            return True  # Don't fail deployment for missing DB in development

        print(f"🔗 [AUTO-MIGRATE] Database URL configured: {database_url[:50]}...")
        ...
        try:
            ...
            print(f"⚠️  [AUTO-MIGRATE] Database modules not available: {e}")
        ...
        if all_present:
            ...
            print(f"⚠️  [AUTO-MIGRATE] Some tables still missing: {missing_after}")
            return False
...
def main():
    ...
    if success:
        print("\n✅ [AUTO-MIGRATE] Database migration completed successfully!")
        print("🚀 [AUTO-MIGRATE] Application is ready to start")
        sys.exit(0)
    else:
        print("\n❌ [AUTO-MIGRATE] Database migration failed!")
