...
def setup_production_environment():
    ...
    # Ensure static files are set up (fallback if heroku-postbuild didn't work)
    static_source = project_root / "dist"
    static_dest = project_root / "backend" / "static"

    if static_source.exists() and not static_dest.exists():
        logger.info("Setting up static files from dist directory...")
    ...
    elif static_dest.exists():
        logger.info("Static files already exist")
    else:
        logger.warning("No static files found - app will run in API-only mode")
