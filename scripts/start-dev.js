...
async function waitForServer(port, path = '/', maxRetries = MAX_RETRIES, name = 'Server') {
...
async function main() {
    ...
    try {
        ...
        const backend = startProcess('python', ['backend/start_dev_server.py'], 'BACKEND', colors.blue);
        ...
        const frontend = startProcess('npm', ['run', 'frontend'], 'FRONTEND', colors.green);
        processes.push(frontend);
        
        // Wait for frontend to be ready
        const frontendReady = await waitForServer(FRONTEND_PORT, '/', 30, 'FRONTEND');
        if (!frontendReady) {
        ...
        const electron = startProcess('cross-env', ['VITE_DEV_SERVER_URL=http://localhost:5173', 'electron', '.'], 'ELECTRON', colors.magenta);
        ...
        log('STARTUP', 'Electron: Desktop application window should open', colors.magenta);
